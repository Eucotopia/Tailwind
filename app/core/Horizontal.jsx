...
function Minimap() {
    ...
    return (
        <group ref={ref}>
            {urls.map((_, i) => (
                <line key={i} geometry={geometry} material={material}
...
function Item({index, position, scale, c = new THREE.Color(), ...props}) {
    ...
    return <Image ref={ref} {...props} position={position} scale={scale} onClick={click} onPointerOver={over}
                  onPointerOut={out}/>
}
...
function Items({w = 0.7, gap = 0.15}) {
    ...
    const xW = w + gap
    return (
        <ScrollControls horizontal={true}  damping={0.1} pages={(width - xW + urls.length * xW) / width}>
            <Minimap/>
            <Scroll>
                {urls.map((url, i) => <Item key={i} index={i} position={[i * xW, 0, 0]} scale={[w, 4, 1]}
                                            url={url}/>) /* prettier-ignore */}
            </Scroll>
        </ScrollControls>
    )
}
